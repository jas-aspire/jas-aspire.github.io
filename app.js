...
/* ── Reliable scroll helper for FireStick ───────────────────────────────
   scrollIntoView() with behavior:'smooth' is broken inside position:fixed
   containers on WebKit/Silk — it tries to scroll the viewport (which has
   overflow:hidden) rather than the actual overflow-y:scroll inner div.
   We bypass it entirely and manipulate scrollTop directly.
   ──────────────────────────────────────────────────────────────────────── */
function _ensureVisible(el) {
  if (!el) return;

  // 1. Vertical — find the active view's scroll container
  const SCROLL_IDS = [
    'home-scroll', 'search-scroll', 'detail-scroll',
    'history-scroll', 'genre-scroll', 'coll-scroll',
    'settings-wrap', 'auth-wrap',
  ];
  let vc = null;
  for (const id of SCROLL_IDS) {
    const s = document.getElementById(id);
    if (s && s.contains(el)) { vc = s; break; }
  }
  if (vc) {
    const er = el.getBoundingClientRect();
    const cr = vc.getBoundingClientRect();
    const PAD = 72; // keep element comfortably off the edge
    if (er.bottom > cr.bottom - PAD) {
      vc.scrollTop += er.bottom - cr.bottom + PAD;
    } else if (er.top < cr.top + PAD) {
      vc.scrollTop = Math.max(0, vc.scrollTop - (cr.top - er.top + PAD));
    }
  }

  // 2. Horizontal — scroll the row-strip so the focused card is visible
  const strip = el.closest('.row-strip, .cast-strip, .detail-more-strip');
  if (strip) {
    const er = el.getBoundingClientRect();
    const sr = strip.getBoundingClientRect();
    const HPAD = 52;
    if (er.right > sr.right - HPAD) {
      strip.scrollLeft += er.right - sr.right + HPAD;
    } else if (er.left < sr.left + HPAD) {
      strip.scrollLeft = Math.max(0, strip.scrollLeft - (sr.left - er.left + HPAD));
    }
  }
}
...
/* ═══════════════════════════════════════════════════════════════════
   ...
   Applies / removes body.perf-mode class which drives all CSS.
   Also stops the hero carousel timer to save CPU cycles.
═══════════════════════════════════════════════════════════════════ */
const PERF_KEY = 'jm_perf_mode';
let _perfMode  = false;
...
function applyPerfMode(on) {
  ...
  if (btn) {
    btn.classList.toggle('perf-active', on);
    btn.textContent = on ? '⚡ Perf: ON' : '⚡ Perf';
  }

  // Persist the preference
  try { localStorage.setItem(PERF_KEY, on ? '1' : '0'); } catch (_) {}

  // In perf mode: kill the hero auto-cycle (saves a timer + DOM mutations every 7 s)
  if (on) {
    stopHeroCycle();
  } else {
    // Only restart if we're currently on the home view
    if (active === 'home') startHeroCycle();
  }

  showToast(on ? '⚡ Performance Mode ON' : '✨ Normal Mode restored', 'neutral');
}
...
function loadPerfMode() {
  try {
    ...
    if (saved === '1') {
      ...
      const btn = document.getElementById('btn-perf-mode');
      if (btn) { btn.classList.add('perf-active'); btn.textContent = '⚡ Perf: ON'; }
    }
  } catch (_) {}
}
...
function startHeroCycle() {
  ...
  if (_perfMode) return; // don't auto-cycle in perf mode — saves a timer + DOM writes
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // Apply saved performance mode preference BEFORE anything renders
  loadPerfMode();
  ...
  document.getElementById('btn-perf-mode').onclick   = () => applyPerfMode(!_perfMode);
